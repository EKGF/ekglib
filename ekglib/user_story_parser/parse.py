...
from ..kgiri import set_kgiri_base, set_kgiri_base_replace, \
    set_cli_params as kgiri_set_cli_params, kgiri_replace_iri_in_graph, kgiri_replace_iri_in_literal
...
class UserStoryParser:
    ...
    def check_story_base_name(self, user_story: rdflib.URIRef):
        ...
        self.add_literal_triple(user_story, USERSTORY.baseName, self.userStoryFile.parent.stem)

    def check_story_sparql(self, user_story: rdflib.URIRef):
        if self.verbose:
            log_item("Looking for triple", EKGPSS.sparqlStatementFileName)
        count = 0
        for sparqlStatementFileName in self.g.objects(user_story, EKGPSS.sparqlStatementFileName):
            count += 1
            self.process_sparql_literal(user_story, self.check_sparql_file_name(sparqlStatementFileName))
        if count == 0:
...
def main():
    ...
    parser.add_argument('--input', '-i', help='The input user-story.ttl file', required=True)
    parser.add_argument('--output', '-o', help='The output user-story.ttl file (with embedded SPARQL)')

...
def set_cli_params(parser):
    ekg_sparql_endpoint = os.getenv('EKG_SPARQL_ENDPOINT', 'http://host.docker.internal:5820')
    ekg_data_source_code = os.getenv('EKG_DATA_SOURCE_CODE', os.getenv('EKG_DATASET_CODE', None))
    user = os.getenv('USER', getpass.getuser())
    if ekg_data_source_code:
        ekg_sparql_database = os.getenv('EKG_SPARQL_DATABASE', f'staging-{ekg_data_source_code}-{user}')
    else:
    ...
    if ekg_sparql_database:
        group.add_argument(
            '--sparql-endpoint-database', '--database',
    ...
    if ekg_sparql_passwd:
        group.add_argument(
            '--sparql-endpoint-passwd', '--sparql-endpoint-password',

...
def pytest_addoption(parser):
    parser.addoption(
        "--run-triple_store", action="store_true", default=False, help="run triple_store tests (SPARQL endpoint at http://localhost:5820)"
    )
    parser.addoption(
        "--run-object_store", action="store_true", default=False, help="run object_store tests (S3 endpoint at http://localhost:9000)"
    )
    parser.addoption(
...
def pytest_configure(config):
    config.addinivalue_line("markers", "triple_store: marks test as a triple_store dependent test (SPARQL endpoint at http://localhost:5820)")
    config.addinivalue_line("markers", "object_store: marks test as an object_store dependent test (S3 endpoint at http://localhost:9000)")
    config.addinivalue_line("markers", "ldap: marks test as an (external) LDAP dependent test")

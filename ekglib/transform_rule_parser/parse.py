...
def add_transform_rule_namespaces(rule_graph: Graph):
...
class TransformRuleParser:
    """Checks each `rule.ttl` file in each subdirectory of `/metadata/transform` to
    see if it refers to `.sparql_endpoint` files that are meant to be used by the "Transform Step".
    ...
    def __init__(self, args, input_file_name=None, rule_file_iri: URIRef = None):
        ...
        self.transform_rule_file = Path(input_file_name)
        if not self.transform_rule_file.exists():
        ...
        if self.args.ontologies_root is None:
            error("Transform Rule Parser requires --ontologies-root to be specified")
        self.ontologies_root = check_ontologies(Path(self.args.ontologies_root))

    def check(self) -> int:
        self.g = self.read_rule_file()
        count = 0
        for transform_rule in self.get_rule_iris():
            if self.check_rule(transform_rule):
                count += 1
        if not count:
            log_error("Could not find any executable rules in {}".format(self.transform_rule_file))
            return 1
    ...
    def read_rule_file(self):
        ...
        add_transform_rule_namespaces(rule_graph)
        load_rdf_file_into_graph(rule_graph, self.transform_rule_file)
        return rule_graph

    def set_key(self):
        return self.transform_rule_file.parent.parent.stem
    ...
    def key(self):
        return f"{self.set_key()}-{self.transform_rule_file.parent.stem}"

    def sort_key(self):
        return f"{self.set_sort_key()}-{self.transform_rule_file.parent.stem}"
    ...
    def check_rule(self, transform_rule_iri) -> bool:
    ...
    def check_sparql_file_name(self, sparql_file_name):
        if self.verbose:
            log_item("Checking", self.transform_rule_file.parent / sparql_file_name)
        sparql_file_name_full_path = self.transform_rule_file.parent / sparql_file_name
        if not sparql_file_name_full_path.exists():
            warning(f"Could not find {sparql_file_name_full_path}")
            return None
        log_item("Found SPARQL file", f"{self.transform_rule_file.parent.name}/{sparql_file_name}")
        return rdflib.Literal(sparql_file_name_full_path.read_text())

    def process_sparql_literal(self, transform_rule, sparql_literal):
...
def runit(args, stream) -> int:
    processor = TransformRuleParser(args, input_file_name=args.input)
    processor.check()
